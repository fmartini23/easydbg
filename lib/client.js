...
class EasyDBGClient {
  ...
  _getFriendlyConnectionErrorMessage(error) {
    ...
    let hint = `Verifique as credenciais (usuário: '${connectionConfig.user}', senha) e o endereço (host: '${connectionConfig.server || connectionConfig.host}') no seu arquivo de configuração.`;

    if (code === 'ENOTFOUND' || code === 'ECONNREFUSED') {
      hint = `Não foi possível conectar ao host "${connectionConfig.server || connectionConfig.host}". O servidor de banco de dados está rodando e acessível pela rede? Verifique o endereço e a porta.`;
    } else if (code === 'ELOGIN') { // MSSQL
      hint = `Falha no login para o usuário '${connectionConfig.user}'. Possíveis causas:\n` +
             `  1. A senha fornecida está incorreta.\n` +
             `  2. O usuário '${connectionConfig.user}' não existe no servidor SQL.\n` +
             `  3. O usuário não tem permissão para conectar a este banco de dados.\n` +
             `  Verifique suas credenciais no arquivo .env ou na configuração.`;
    } else if (code === 'ER_ACCESS_DENIED_ERROR') { // MySQL
      hint = `Acesso negado para o usuário '${connectionConfig.user}'. Verifique se o usuário e a senha estão corretos.`;
    ...
    } else if (code === 'ER_BAD_DB_ERROR' || (originalError.message && originalError.message.includes('database') && originalError.message.includes('does not exist'))) { // MySQL e Postgres
  ...
  _getFriendlyQueryErrorMessage(error, sql) {
    ...
    if (code === 'ER_NO_SUCH_TABLE' || (originalError.message && originalError.message.includes('does not exist'))) {
      ...
      hint = `A coluna "${columnName}" não foi encontrada em uma das tabelas da sua consulta. Verifique se o nome da coluna está correto.`;
      ...
      hint = 'A consulta contém um erro de sintaxe SQL. Verifique a estrutura da sua query.';
    }
